...
const saveDataCalender = async() => {
  for (let i = 1; i <= 10; i++) {
    ...
    await pool.query(`COPY booking.booking (id, price, stars, weeklyViewCount, serviceFee, weeklyDiscountPercent, isRareFind, cleaningFee) FROM \'${file}\' DELIMITERS ',' CSV HEADER;`, (err) => {
      let after = Date.now();
      if (err) {
        console.log('error', err);
      } else {
